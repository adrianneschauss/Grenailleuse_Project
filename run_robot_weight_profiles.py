...
def run_profile(profile, n_runs, seed_base):
    ...
    for i in range(n_runs):
        ...
        result = run_robot(
            plot=False,
            variable_speed=True,
            s=0.15,  # fixed as requested
            speed_ctrl_w_step_out=profile["w_step_out"],
            speed_ctrl_w_robot_out=profile["w_robot_out"],
            speed_ctrl_w_conv_out=profile["w_conv_out"],
...
def interpolate_profiles(anchor_profiles, points_between=2):
    keys = ["w_step_out", "w_robot_out", "w_conv_out", "w_inspect"]
    out = []
...
def main():
    ...
    for idx, profile in enumerate(profiles, start=1):
        metrics = run_profile(profile=profile, n_runs=n_runs, seed_base=25000 + idx * 100)
        rows.append(
            {
                "case": profile["name"],
                ...
                "w_robot_out": profile["w_robot_out"],
                "w_conv_out": profile["w_conv_out"],
    ...
    csv_path = "robot_weight_profiles_s01.csv"
    ...
    ax1.set_title("Verticale: profils de poids (inspecteur=0.05, grenailleuse=0.15)")
    ...
    fig.tight_layout()
    fig_path = rf"robot_weight_profiles_005_015.png"
