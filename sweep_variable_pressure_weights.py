...
def run_stats(n_runs, seed_base, weights, base_kwargs):
    ...
    for i in range(n_runs):
        ...
        result = run_variable(
            ...
            speed_ctrl_w_var=weights[1],
            speed_ctrl_w_cont=weights[2],
...
def main():
    ...
    for i, p in enumerate(top, start=1):
        ...
        print(
            ...
            f"weights=({p['w_step_out']:.3f}, {p['w_var']:.3f}, {p['w_cont']:.3f}, {p['w_inspect']:.3f}){tag}"
        )

    csv_path = "sweep_variable_pressure_weights.csv"
    ...
    ax.set_title("Variable conveyor flow: pressure-weight sweep")
    ax.grid(True, alpha=0.3)
    ax.legend()
    cbar = plt.colorbar(sc, ax=ax)
    cbar.set_label("w_inspect")
    plt.tight_layout()
    fig_path = "sweep_variable_pressure_weights.png"
