...
def demo_composite_flow(
    ...
    t_dis=None,
    t_dis2=None,
    t_prevar_to_var=None,
    t_prevar_exit_to_var_entry=None,
    t_stepout_to_prevar=None,
    t_var_exit_to_cont_entry=None,
    t_sink_to_inspector=None,
    inspect_min=None,
    inspect_max=None,
    max_long = None,
    min_long = None,
    ...
    speed_ctrl_tempon_target_fill=None,
    speed_ctrl_tempon_bias_gain=None,
    ...
    if t_prevar_to_var is None:
        t_prevar_to_var = getattr(Parameter_horizontal, "t_prevar_to_var", 0.5)
    if t_prevar_exit_to_var_entry is None:
        t_prevar_exit_to_var_entry = getattr(
            Parameter_horizontal,
            "t_prevar_exit_to_var_entry",
            0.5,
        )
    if t_stepout_to_prevar is None:
        t_stepout_to_prevar = getattr(Parameter_horizontal, "t_stepout_to_prevar", 0.5)
    if t_var_exit_to_cont_entry is None:
        t_var_exit_to_cont_entry = getattr(
            Parameter_horizontal,
            "t_var_exit_to_cont_entry",
            0.5,
        )
    if t_sink_to_inspector is None:
        t_sink_to_inspector = getattr(Parameter_horizontal, "t_sink_to_inspector", 0.5)
    ...
    if speed_ctrl_tempon_target_fill is None:
        speed_ctrl_tempon_target_fill = getattr(Parameter_horizontal, "speed_ctrl_tempon_target_fill", 0.7)
    if speed_ctrl_tempon_bias_gain is None:
        speed_ctrl_tempon_bias_gain = getattr(Parameter_horizontal, "speed_ctrl_tempon_bias_gain", 0.35)
...
# first step conveyor (grenailleuse)
    ...
    grenailleuse_blocked_intervals = []
    ...
    env.process(
        step_conveyor_advance(
            ...
            blocked_intervals=grenailleuse_blocked_intervals,
        )
    )
    ...
    bottle_history = defaultdict(dict)
    active_inspector = {"item": None}
    position_log = {
        "t": [],
        "positions": [],
        "position_items": [],
    ...
    def record_event(item, stage, now):
        bottle_id = item_id(item)
        if bottle_id is None:
            return
        bottle_history[int(bottle_id)].setdefault(stage, float(now))

    pre_position_state = {"positions": [], "items": []}
    def pre_conveyor_logger(now, segment_id, items, segment_length=None):
        pre_position_state["positions"] = [item["pos"] for item in items]
        pre_position_state["items"] = [
            {"id": item_id(item), "pos": item["pos"]} for item in items
        ]
        for item in items:
            record_event(item, "pre_var_entry", now)
    ...
    env.process(
        continuous_conveyor(
            ...
            load_delay=t_stepout_to_prevar,
        )
    )
    ...
    def position_logger(
        ...
        position_log["position_items"].append(
            [{"id": item_id(item), "pos": item["pos"]} for item in items]
        )
        position_log["cont_positions"].append(
            [item["pos"] for item in cont_items_state["items"]]
        )
        position_log["cont_items"].append(
            [{"id": item_id(item), "pos": item["pos"]} for item in cont_items_state["items"]]
        )
        position_log["pre_positions"].append(pre_position_state["positions"])
        position_log["pre_items"].append(list(pre_position_state["items"]))
        position_log["pre_var_buffer_items"].append(
            [{"id": item_id(item)} for item in pre_var_in.items]
        )
        position_log["inspect_count"].append(len(det1_hold.items))
        position_log["inspect_items"].append(
            [{"id": item_id(item)} for item in det1_hold.items]
        )
        ...
        inspector_item = active_inspector["item"]
        position_log["inspector_item"].append(
            item_id(inspector_item) if inspector_item is not None else None
        )
        position_log["post_inspect_items"].append(
            [{"id": item_id(item)} for item in post_inspect.items]
        )
        position_log["step_output_items"].append(
            [{"id": int(item)} for item in step_g["output_store"].items]
        )
    ...
    def variable_conveyor_segment(
        ...
        while True:
            ...
            if (
                ...
                if t_prevar_to_var > 0:
                    yield env.timeout(t_prevar_to_var)
                item = yield input_store.get()
                record_event(item, "pre_var_exit", env.now)
                if t_prevar_exit_to_var_entry > 0:
                    yield env.timeout(t_prevar_exit_to_var_entry)
                items.append(
                    {"payload": item, "pos": 0.0, "entry_start": env.now}
                )
                record_event(item, "variable_entry", env.now)
            if step_mode:
                step_acc += dt
    ...
    def grenailleuse_speed_controller(
        ...
        while True:
            ...
            # Bias pressure so controller prefers keeping tempon (pre_var) near target fill.
            # If tempon fill is below target, effective pressure decreases -> faster step time.
            tempon_fill_error = float(speed_ctrl_tempon_target_fill) - pre_var_fill
            effective_pressure = downstream_pressure - float(speed_ctrl_tempon_bias_gain) * tempon_fill_error
            if effective_pressure < 0.0:
                effective_pressure = 0.0
            elif effective_pressure > 1.0:
                effective_pressure = 1.0
            pressure_window.append(effective_pressure)
            ...
            if (not items or items[0]["pos"] >= spacing) and len(items) < max_items:
                ...
                for i, src in enumerate(src_items):
                    if src["pos"] >= length_second - 1e-6:
                        src_items.pop(i)
                        record_event(src["payload"], "variable_exit", env.now)
                        if t_var_exit_to_cont_entry > 0:
                            yield env.timeout(t_var_exit_to_cont_entry)
                        items.append({"payload": src["payload"], "pos": 0.0, "end_start": None})
                        record_event(src["payload"], "continuous_entry", env.now)
                        break
            for item in items:
                item["pos"] += speed * dt
            remaining = []
            for item in items:
                if item["pos"] >= length:
                    if item["end_start"] is None:
                        item["end_start"] = env.now
                    if env.now - item["end_start"] < t_dis2:
                        item["pos"] = length
                        remaining.append(item)
                    elif len(end_store.items) < end_store.capacity:
                        payload = item["payload"]
                        if not isinstance(payload, dict) or "id" not in payload:
                            payload = {"id": payload}
                        payload["hold_start"] = env.now
                        record_event(payload, "sink_entry", env.now)
    ...
    inspected_times = []
    busy_time = [0.0]
    def on_inspect_start(item, now):
        active_inspector["item"] = item
        record_event(item, "inspector_start", now)

    def on_inspect_complete(item, now):
        record_event(item, "inspector_complete", now)
        record_event(item, "post_inspect_entry", now)
        active_inspector["item"] = None

    def on_post_release(item, now):
        record_event(item, "post_inspect_exit", now)

    def inspector_process_with_sink_delay():
        while True:
            while not det1_hold.items:
                yield env.timeout(0.01)
            with inspector.request() as req:
                yield req
                if t_sink_to_inspector > 0:
                    yield env.timeout(t_sink_to_inspector)
                item = yield det1_hold.get()
                start = env.now
                on_inspect_start(item, start)
                yield env.timeout(inspect_time(inspect_min, inspect_max, s, max_long, min_long))
                end = env.now
                busy_time[0] += end - start
            on_inspect_complete(item, end)
            yield post_inspect.put(item)
            inspected_times.append(env.now)

    env.process(
        inspector_process_with_sink_delay()
    )
    env.process(unload_delay(env, post_inspect, final_store, post_inspect_delay, on_release=on_post_release))
    ...
    monitor = {
        ...
        "grenailleuse_output_full": [],
        "grenailleuse_blocked": [],
    }
    ...
    def monitor_process():
        while True:
            ...
            monitor["step_output_count"].append(len(step_g["output_store"].items))
            monitor["pre_var_in_count"].append(len(pre_var_in.items))
            ...
            out_full = int(
                len(step_g["output_store"].items) >= step_g["output_store"].capacity
            )
            monitor["grenailleuse_output_full"].append(out_full)
            # Must match KPI used by grenailleuse_blocked_time in step_conveyor_advance
            # (and therefore compare_runs): blocked iff output handoff is full.
            monitor["grenailleuse_blocked"].append(out_full)
    ...
    def step_position_monitor():
        while True:
            ...
            for slot_idx, bottle_id in enumerate(step_g["slots"]):
                if bottle_id is not None:
                    bottle_history[int(bottle_id)].setdefault("grenailleuse_entry", float(env.now))
                    bottle_history[int(bottle_id)]["grenailleuse_slot"] = int(slot_idx)
                    bottle_history[int(bottle_id)]["grenailleuse_t"] = float(env.now)
            for bottle_id in step_g["output_store"].items:
                bottle_history[int(bottle_id)].setdefault("step_output_entry", float(env.now))
            yield env.timeout(sample_time)
    ...
    return {
        ...
        "grenailleuse_blocked_intervals": grenailleuse_blocked_intervals,
        ...
        "monitor": monitor,
        "bottle_history": {int(k): dict(v) for k, v in bottle_history.items()},
