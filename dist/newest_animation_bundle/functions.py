...
def step_conveyor_advance(
    env,
    step_conveyor,
    gr_conv,
    exit_times=None,
    blocked_time=None,
    wait_reason_time=None,
    wait_reason_count=None,
    ...
    while True:
        ...
        if not any_items or output_full:
            reason = "output_full" if output_full else "empty_step"
            if wait_reason_time is not None:
                wait_reason_time[reason] = (
                    wait_reason_time.get(reason, 0.0) + float(step_conveyor["step_time"])
                )
            if wait_reason_count is not None:
                wait_reason_count[reason] = wait_reason_count.get(reason, 0) + 1
            if output_full and blocked_time is not None:
                blocked_time[0] += step_conveyor["step_time"]
            if output_full and blocked_intervals is not None:
                start_t = float(env.now)
                end_t = start_t + float(step_conveyor["step_time"])
                blocked_intervals.append((start_t, end_t))
...
def unload_delay(env, input_store, output_store, delay_time, on_release=None):
        while True:
            item = yield input_store.get()
            yield env.timeout(delay_time)
            if on_release is not None:
                on_release(item, env.now)
            yield output_store.put(item)
