...
step_time = 12 #according to  #max is 12 and min 72
...
length_first = 151.4
length_second = 395 #sink is included at the end?
length_third = 93
...
t_dis = 3
t_dis2 = 3
# Transfer delay from the end of the tempon into the variable conveyor.
t_prevar_to_var = 0.5
# Transfer delay after leaving the pre-var exit slot before appearing on variable entry.
t_prevar_exit_to_var_entry = 0.5
# Transfer delay from grenailleuse step output into the pre-variable conveyor.
t_stepout_to_prevar = 0.5
# Transfer delay after leaving the variable conveyor before appearing on the continuous conveyor.
t_var_exit_to_cont_entry = 0.5
# Transfer delay from sink/buffer into the inspector.
t_sink_to_inspector = 0.5
...
speed_ctrl_fast_factor = 0.95
speed_ctrl_max_step_mult = 3
...
# target_fill is normalized 0..1 occupancy for pre_var_in.
# bias_gain controls how strongly low fill asks for higher grenailleuse speed.
speed_ctrl_tempon_target_fill = 0.55
speed_ctrl_tempon_bias_gain = 0.0

# Shared grenailleuse output handoff capacity across flows.
# Keep at 1 to avoid adding extra buffer space.
step_output_capacity = 1
