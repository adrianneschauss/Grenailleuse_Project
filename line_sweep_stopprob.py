...
def main():
    ...
    series = [
        ...
        ("Robot (var)", "tab:blue", True, 1100, run_robot),
        ("Variable", "tab:orange", False, 2000, run_variable),
        ("Variable (var)", "tab:orange", True, 2100, run_variable),
        ("Tampon", "tab:green", False, 3000, run_tempon),
        ("Tampon (var)", "tab:green", True, 3100, run_tempon),
    ]

    results = {}
    for label, color, var_speed, seed, fn in series:
        results[label] = sweep_stop_prob(fn, p_vals, s_fixed, n_runs, seed, variable_speed=var_speed)

    fig, axes = plt.subplots(3, 1, figsize=(9, 10), sharex=True, constrained_layout=True)

    for label, color, var_speed, _, _ in series:
        data = results[label][0]
        mean = data[:, 0]
        ci = data[:, 1]
        linestyle = "--" if var_speed else "-"
    ...
    for label, color, var_speed, _, _ in series:
        ...
        ci = data[:, 1]
        linestyle = "--" if var_speed else "-"
        axes[1].plot(p_vals, mean, label=label, color=color, linestyle=linestyle)
        axes[1].fill_between(p_vals, mean - ci, mean + ci, color=color, alpha=0.15)
    ...
    for label, color, var_speed, _, _ in series:
        ...
        ci = data[:, 1]
        linestyle = "--" if var_speed else "-"
        axes[2].plot(p_vals, mean, label=label, color=color, linestyle=linestyle)
        axes[2].fill_between(p_vals, mean - ci, mean + ci, color=color, alpha=0.15)
